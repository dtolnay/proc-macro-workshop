...
use syn::{
    ...
    Generics, Ident, Lit, Meta, NestedMeta, Result, Token, TypePath, WherePredicate,
};
...
fn custom_clauses(attrs: &[Attribute]) -> Result<Option<Vec<WherePredicate>>> {
    ...
    for attr in attrs {
        if attr.path.is_ident("debug") {
            ...
            for elem in nested {
                let lit = match &elem {
                    NestedMeta::Meta(Meta::NameValue(nv)) if nv.path.is_ident("bound") => &nv.lit,
                    _ => return Err(error()),
                };
                ...
                let parsed = string.parse_with(Punctuated::<_, Token![,]>::parse_terminated)?;
