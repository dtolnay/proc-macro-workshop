...
#[bitfield]
pub struct UsesBoolAsSpecifier {
    a: A,
    b: B,
    c: C,
    d: D,
...
fn main() {
    let mut x = UsesBoolAsSpecifier::new();
    ...
    let a = true;
    x.set_a(a); // expect fn(&mut UsesBoolAsSpecifier, bool)
    let b = 1;
    x.set_b(b);
    let c = 1;
    x.set_c(c);
    let d = 1;
    x.set_d(d); // expect fn(&mut UsesBoolAsSpecifier, u32)
    ...
    assert_eq!(size_of_val(&x.get_a()), 1); // expect fn(&UsesBoolAsSpecifier) -> bool
    assert_eq!(size_of_val(&x.get_b()), 1);
    assert_eq!(size_of_val(&x.get_c()), 1);
    assert_eq!(size_of_val(&x.get_d()), 4); // expect fn(&UsesBoolAsSpecifier) -> u32
}
