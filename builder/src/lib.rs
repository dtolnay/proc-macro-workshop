...
use quote::quote;
use syn::{
    parse_macro_input, Data, DeriveInput, Error, Field, Fields, Ident, Lit, Meta, MetaNameValue,
    ...
    fn try_from(field: &Field) -> Result<Self> {
        ...
        for attr in &field.attrs {
            ...
            for nested in &meta.nested {
                let lit = match nested {
                    NestedMeta::Meta(Meta::NameValue(MetaNameValue {
                        path,
                        lit: Lit::Str(lit),
                        ..
                    })) if path.is_ident("each") => lit,
                    _ => return Err(Error::new_spanned(&meta, expected)),
                };
                each = Some(lit.parse()?);
            }
        }
